#==============================================================================
# Bag Graphical Overhaul by LackDeJurane or CharizardThree3
# Made compatible for v21.1 by A.I.R 
...
class PokemonBag_Scene
  ...
  def pbStartScene(bag,choosing=false,filterproc=nil,resetpocket=true)
    ...
    @sliderbitmap = AnimatedBitmap.new(_INTL("Graphics/UI/Bag/icon_slider"))
    @pocketbitmap = AnimatedBitmap.new(_INTL("Graphics/UI/Bag/icon_pocket"))
    @sprites = {}
    @sprites["grid"]=AnimatedPlane.new(@viewport) 
    if $player.female?
      @sprites["grid"]=AnimatedPlane.new(@viewport)  
      @sprites["grid"].bitmap = Bitmap.new("Graphics/UI/Bag/bg_gridf")
    else
      @sprites["grid"]=AnimatedPlane.new(@viewport)
      @sprites["grid"].bitmap = Bitmap.new("Graphics/UI/Bag/bg_grid") 
    end  
    ...
    @sprites["switchL"].setBitmap("Graphics/UI/Bag/switchL")
    @sprites["switchL"].x=2
    @sprites["switchL"].y=180
    @sprites["switchR"]=IconSprite.new(0,0,@viewport)
    @sprites["switchR"].setBitmap("Graphics/UI/Bag/switchR")
  ...
  def showPocketAnimation
    if $player.female?
      @sprites["bagsprite"].setBitmap("Graphics/UI/Bag/bag_#{@bag.last_viewed_pocket}_fm") 
       
      @sprites["bagsprite"].setBitmap("Graphics/UI/Bag/bag_#{@bag.last_viewed_pocket}_f")
    else
      @sprites["bagsprite"].setBitmap("Graphics/UI/Bag/bag_#{@bag.last_viewed_pocket}m")
  ...
  def pbRefresh
    ...
    if $player.female?
      @sprites["background"].setBitmap(sprintf("Graphics/UI/Bag/bg_f"))
    else
      @sprites["background"].setBitmap(sprintf("Graphics/UI/Bag/bg"))
    end
    # Set the bag sprite
        ...
        if itemwindow.sorting
          ...
          if Input.trigger?(Input::LEFT)
            ...
            if itemwindow.pocket!=newpocket
              ...
              thispocket = @bag.pockets[itemwindow.pocket]
              showPocketAnimation
              @sprites["switchL"].setBitmap("Graphics/UI/Bag/switchL2") if USEARROWS
              
              @sprites["switchL"].setBitmap("Graphics/UI/Bag/switchL") if  USEARROWS
            ...
            if itemwindow.pocket!=newpocket
              ...
              thispocket = @bag.pockets[itemwindow.pocket]
              showPocketAnimation
              @sprites["switchR"].setBitmap("Graphics/UI/Bag/switchR2") if USEARROWS
              
              @sprites["switchR"].setBitmap("Graphics/UI/Bag/switchR") if USEARROWS
              pbSEPlay("BW2BagSound")
              pbRefresh
            end
